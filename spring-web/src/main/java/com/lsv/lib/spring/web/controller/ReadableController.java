...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    @GetMapping(PARAM_ID)
    default IN findById(@PathVariable ID id) {
        return mappableOf(service().findById(Identifiable.of(outClass(), id)).orElseThrow());
    }

    @SuppressWarnings("unchecked")
    @GetMapping
    default Page<IN> findByFilter(@PageableDefault(
        page = PAGEABLE_DEFAULT_PAGE,
        size = PAGEABLE_DEFAULT_SIZE,
        sort = PAGEABLE_DEFAULT_SORT,
        direction = Sort.Direction.DESC) Pageable pageable) {

        return (Page<IN>) ConverterSpringJpa.to(
