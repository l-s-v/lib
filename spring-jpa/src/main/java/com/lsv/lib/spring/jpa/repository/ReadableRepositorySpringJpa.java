...
public interface ReadableRepositorySpringJpa
    ...
    ProviderRepositorySpringJpa<I, ID, P, S> {
    ...
    default ListDto<I> findByFilter(@NonNull Filter<I> filter) {
        ...
        if (specification == null && filter.obj() != null) {
            example = Example.of(this.mapper().to(filter.obj()));
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    private ListDto<I> findAllPageable(Example<P> example,
        ...
        if (specification != null) {
            page = this.storable().findAll(specification, pageable);
        } else if (example != null) {
            page = this.storable().findAll(example, pageable);
        } else {
            page = this.storable().findAll(pageable);
        }

        return Optional.of(page)
            .map(ps -> ListDto
                .of(this.mapper().of(ps.getContent()))
    ...
    private ListDto<I> findAllSorted(Example<P> example,
        ...
        if (specification != null) {
            results = this.storable().findAll(specification, sort);
        } else if (example != null) {
            results = this.storable().findAll(example, sort);
        } else {
            results = this.storable().findAll(sort);
        }

        return Optional.of(results)
            .map(ps -> ListDto
                .of(this.mapper().of(ps))
                .get())
            .orElse(ListDto.empty());
    }
}